...
pub fn distance_of_sqs(sq_one: SQ, sq_two: SQ) -> u8 {
    ...
    unsafe {
        DISTANCE_TABLE[sq_one.0 as usize][sq_two.0 as usize]
    }
}
...
pub fn line_bb(sq_one: SQ, sq_two: SQ) -> u64 {
    ...
    debug_assert!(sq_two.is_okay());
    unsafe {
       *(LINE_BITBOARD.get_unchecked(sq_one.0 as usize)).get_unchecked(sq_two.0 as usize)
    }
}
...
pub fn adjacent_sq_file(sq: SQ) -> u64 {
    debug_assert!(sq.is_okay());
    unsafe {
        *ADJACENT_FILES_BB.get_unchecked(sq.file() as usize)
    }
}
...
pub fn adjacent_file(f: File) -> u64 {
    unsafe {
        *ADJACENT_FILES_BB.get_unchecked(f as usize)
    }
}
...
pub fn pawn_attacks_from(sq: SQ, player: Player) -> u64 {
    debug_assert!(sq.is_okay());
    unsafe {
        *PAWN_ATTACKS_FROM.get_unchecked(player as usize).get_unchecked(sq.0 as usize)
    }
}
...
pub fn ring_distance(sq: SQ, distance: u8) -> u64 {
    ...
    unsafe {
        *DISTANCE_RING_TABLE.get_unchecked(distance as usize).get_unchecked(sq.0 as usize)
    }
}
...
pub fn forward_rank_bb(player: Player, rank: Rank) -> u64 {
    unsafe {
        *FORWARD_RANKS_BB.get_unchecked(rank as usize).get_unchecked(player as usize)
    }
}
...
pub fn pawn_attacks_span(player: Player, sq: SQ) -> u64 {
    debug_assert!(sq.is_okay());
    unsafe {
        *PAWN_ATTACKS_SPAN.get_unchecked(player as usize).get_unchecked(sq.0 as usize)
    }
}
...
pub fn forward_file_bb(player: Player, sq: SQ) -> u64 {
    debug_assert!(sq.is_okay());
    unsafe {
        *FORWARD_FILE_BB.get_unchecked(player as usize).get_unchecked(sq.0 as usize)
    }
}
...
pub fn passed_pawn_mask(player: Player, sq: SQ) -> u64 {
    debug_assert!(sq.is_okay());
    unsafe {
        *PASSED_PAWN_MASK.get_unchecked(player as usize).get_unchecked(sq.0 as usize)
    }
}
    ...
    for i in 0..64 as u8 {
        for j in 0..64 as u8 {
            ...
            if rook_attacks(0, i) & j_bb != 0 {
                ...
                BETWEEN_SQUARES_BB[i as usize][j as usize] = rook_attacks(i_bb, j) &
                    rook_attacks(j_bb, i);
            } else if bishop_attacks(0, i) & j_bb != 0 {
                LINE_BITBOARD[i as usize][j as usize] |=
                    (bishop_attacks(0, j) & bishop_attacks(0, i)) | i_bb | j_bb;
                BETWEEN_SQUARES_BB[i as usize][j as usize] = bishop_attacks(i_bb, j) &
                    bishop_attacks(j_bb, i);
            } else {
...
unsafe fn gen_pawn_attacks_span() {
    for p in 0..2 {
        for s in 0..64 {
            FORWARD_FILE_BB[p][s] = FORWARD_RANKS_BB[rank_idx_of_sq(s as u8) as usize][p] & FILE_BB[file_idx_of_sq(s as u8) as usize];
            PAWN_ATTACKS_SPAN[p][s] = FORWARD_RANKS_BB[rank_idx_of_sq(s as u8) as usize][p] & ADJACENT_FILES_BB[file_idx_of_sq(s as u8) as usize];
